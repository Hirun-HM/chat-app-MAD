...
# Test QR Code Contact Shariecho ""
...
if [ "$chatSuccess" = "true" ]; then
    ...
    echo "   📄 $chatResponse"
fiecho "🧪 Testing QR Code Contact Sharing Feature with Home Page Integration"
...
echo ""
echo "3️⃣ Testing chat creation API (for auto-redirect after QR scan)..."
chatResponse=$(curl -s -X POST "http://192.168.1.3:8000/api/chats" \
  -H "Content-Type: application/json" \
  -d '{
    "participants": [1, 2],
    "type": "individual",
...
chatSuccess=$(echo "$chatResponse" | jq -r '.success // false')
if [ "$chatSuccess" = "true" ] || echo "$chatResponse" | grep -q "chatId\|id"; then
    echo "✅ Chat creation API working"
    echo "   � Chat auto-creation after QR scan will work"
else
    echo "⚠️  Chat may already exist or API response format different"
...
echo "   • Chat creation API: $([ "$chatSuccess" = "true" ] && echo "✅ Working" || echo "⚠️  Check needed")"
